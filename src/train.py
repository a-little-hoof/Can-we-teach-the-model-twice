...
def main():
	...
	wandb.init(name=name+"_second"+"-adapter" if model_args.use_adapter else name+"-lora" if model_args.use_lora else name)

	'''
		load datasets
	'''
	if data_args.dataset_name=="ledgar":
		raw_datasets = get_dataset(data_args.dataset_name,True)
	else:
